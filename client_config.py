...
class client_config:
    ...
    # remote_rpcurl = "http://127.0.0.1:8547"  # 采用rpc通信时，节点的rpc端口,和要通信的节点*必须*一致,如采用channel协议通信，这里可以留空
    # channel_host = "127.0.0.1"  # 采用channel通信时，节点的channel ip地址,如采用rpc协议通信，这里可以留空
    # channel_port = 20202  # 节点的channel 端口,如采用rpc协议通信，这里可以留空
    # channel_ca = sdk_dir + "/bin/ca.crt"  # 采用channel协议时，需要设置链证书,如采用rpc协议通信，这里可以留空
    # channel_node_cert = sdk_dir + "/bin/sdk.crt"  # 采用channel协议时，需要设置sdk证书,如采用rpc协议通信，这里可以留空
    # channel_node_key = sdk_dir + "/bin/sdk.key"  # 采用channel协议时，需要设置sdk私钥,如采用rpc协议通信，这里可以留空
    ...
    contract_info_file = sdk_dir + "/bin/contract.ini"  # 保存已部署合约信息的文件
    account_keyfile_path = sdk_dir + "/bin/accounts"  # 保存keystore文件的路径，在此路径下,keystore文件以 [name].keystore命名
    account_keyfile = "0x5360cf77373e8e2eea6c59e3361948094a7e1c09.keystore"
    ...
    solc_path = sdk_dir + "/bin/solc/v0.4.25/solc"
    gm_solc_path = sdk_dir + "/bin/solc/v0.4.25/solc-gm"
    solcjs_path = sdk_dir + "/solcjs"

    logdir = sdk_dir + "/bin/logs"  # 默认日志输出目录，该目录必须先建立
